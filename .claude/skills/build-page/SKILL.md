...
If the page does two of these (e.g., shows a reference table but also edits inventory), pick the section that matches the user's *primary* intent and mention the trade-off in your summary.
...
- If a *specific* inner container needs constraint (a form, a text-heavy reference block), put `max-w-2xl` or `max-w-3xl` on that container — not the root. See conventions.md "When to constrain width" for the full mapping of content type → strategy.
...
The Clear button is *always rendered*, with `isDisabled={!hasFilters}` controlling its state — not `{hasFilters && <Button>Clear</Button>}`. Conditional rendering causes layout shift when filters are applied/cleared, which is jarring.
