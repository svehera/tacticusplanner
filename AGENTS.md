...
- Some characters' abilities have `damageProfile`/`damageProfile_2` constants (in `new-ability-data.json`) that describe damage they _reduce or react to_ rather than damage they deal (e.g. Kîmm's `ModifiedExoarmour`, a defensive passive). This causes false positives in "does this character deal damage type X" checks (e.g. LRE "deals damage of profile type" restrictions). Add such cases to `src/fsd/4-entities/character/damage-profile-exclusions.ts` and always check damage-type membership through its `characterDealsDamageType` helper rather than reading `char.damageTypes.all` directly — that's the single shared place `lre/model/filters.ts`'s `byDamageType` (and any future consumer, e.g. `plan-quests`) should consult.
