...
- A lot of the files have corresponding TypeScript types. Unfortunately the data is not necessarily validated against these types. Either
    - Make `zod` schemas for the data and validate when they are loaded, or
    - Convert the data files to TypeScript files and use `as const` so that TS can validate them at compile time.
