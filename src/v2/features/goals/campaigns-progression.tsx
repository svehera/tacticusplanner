...
export class BattleSavings {
  ...
  /** The savings on our goals by beating only this node. */
  savings: number = 0;
  ...
  public constructor(battle: ICampaignBattleComposed,
    savings: number,
...
export class CampaignProgressData {
   ...
   * The key is the goal ID, not the unit ID.
   */
...
export class CampaignsProgressionService {
  ...
  private static getFactionCampaignMappings(): CampaignFactionMapping {
    var result = new CampaignFactionMapping();
  ...
  private static computeGoalCostsAndUnbeatenNodes(
    ...
    nodesToBeat: Map<string, ICampaignBattleComposed[]>): void {
    ...
    for (const goal of goals) {
      ...
      var unit = StaticDataService.getUnit(goal.unitId);
      if (!unit) {
      ...
      // campaign results.
      console.log("Adding goal data for '" + goal.unitId + "' in '" + unit.faction + "' to campaigns.");
      ...
      goalData.farmData.forEach((data, material) => {
        if (result.materialFarmData.has(material)) {
          var existingData = result.materialFarmData.get(material)!;
   ...
   */
  private static uniquifyNodesToBeat(nodes: Map<string, ICampaignBattleComposed[]>)
    : Map<string, ICampaignBattleComposed[]> {
    var ret = new Map<string, ICampaignBattleComposed[]>;
    for (const [campaign, battles] of nodes.entries()) {
      var newBattles: ICampaignBattleComposed[] = [];
      var used: Set<number> = new Set<number>();
  ...
  public static computeCampaignsProgress(
    goals: Array<ICharacterUpgradeRankGoal | ICharacterUpgradeMow | ICharacterUnlockGoal | ICharacterAscendGoal>,
    campaignProgress: ICampaignsProgress,
    inventoryUpgrades: Record<string, number>): CampaignsProgressData {
    var result = new CampaignsProgressData();

    // For each campaign, a list of battles to beat that will make
    // our goals cheaper.
    var nodesToBeat = new Map<string, ICampaignBattleComposed[]>();
    ...
    this.computeGoalCostsAndUnbeatenNodes(
      goals, campaignProgress, {}, result, nodesToBeat);

    nodesToBeat = this.uniquifyNodesToBeat(nodesToBeat);

    var newMaterialEnergy = new Map<string, number>();
    for (const campaign of nodesToBeat.keys()) {
      if ((nodesToBeat.get(campaign)?.length ?? 0) == 0) continue;
      var nodes: ICampaignBattleComposed[] = nodesToBeat.get(campaign)!.sort((a, b) => { return a.nodeNumber - b.nodeNumber; });
      var cumulativeSavings: number = 0;
      ...
      for (const battle of nodes) {
        if (!result.materialFarmData.has(battle.reward)) {
          console.error("  No data for material '" + battle.reward + "' from " + battle.id + "'.");
          continue;
        }
        const farmData = result.materialFarmData.get(battle.reward)!;
        const newEnergyCost: number = this.getCostToFarmMaterial(
          battle.campaignType, farmData.count, battle.rarityEnum);
        var oldEnergy = newMaterialEnergy.get(battle.reward) ?? farmData.totalEnergy;
        ...
        if (oldEnergy > newEnergyCost) {
          ...
          result.data.get(campaign)?.savings.push(new BattleSavings(battle, farmData.totalEnergy - newEnergyCost, cumulativeSavings));
  ...
  private static addToMaterials(materialReqs: MaterialRequirements, material: string, count: number): void {
    if (materialReqs.materials.hasOwnProperty(material)) {
  ...
  public static computeGoalCost(
    goal: ICharacterUpgradeRankGoal | ICharacterUpgradeMow | ICharacterUnlockGoal,
    campaignProgress: ICampaignsProgress,
    inventoryUpgrades: Record<string, number>): GoalData {

    var materialReqs = new MaterialRequirements();
    if (goal.type == PersonalGoalType.Unlock) {
      var unlockGoal = goal as ICharacterUnlockGoal;
      this.addToMaterials(materialReqs, goal.unitName, charsUnlockShards[goal.rarity] - goal.shards);
    } else {
      const upgradeRanks = goal.type === PersonalGoalType.UpgradeRank
        ? UpgradesService.getCharacterUpgradeRank(goal)
        : UpgradesService.getMowUpgradeRank(goal);
      ...
      for (const unitUpgrade of upgradeRanks) {
        for (const upgradeMaterial of unitUpgrade.upgrades) {
          const expandedRecipe: IRecipeExpandedUpgrade
            = UpgradesService.recipeExandedUpgradeData[upgradeMaterial];
    ...
    var result: GoalData = new GoalData();

    {
      var newMaterials: Record<string, number> = {};
      for (const [material, count] of Object.entries(materialReqs.materials)) {
        var newCount: number = count;
        if (inventoryUpgrades.hasOwnProperty(material)) {
    ...
    var sortedMaterials: string[] =
      Object.keys(materialReqs.materials).sort(
        (a, b) => UpgradesService.recipeExandedUpgradeData[b].rarity - UpgradesService.recipeExandedUpgradeData[a].rarity);
    for (const material of sortedMaterials) {
      const count: number = materialReqs.materials[material];
      var farmData: FarmData = this.getCostToFarm(material, count, campaignProgress, inventoryUpgrades);
  ...
  public static getCostToFarm(
    ...
    count: number,
    campaignProgress: ICampaignsProgress,
    inventoryUpgrades: Record<string, number>): FarmData {
    var result: FarmData = {
      material: material,
      totalEnergy: 0,
      count: count,
    ...
    const hasEarly = result.farmableLocations.filter((x) => x.campaign == 'Indomitus' && x.type == CampaignType.Normal && x.nodeNumber < 30).length > 0;
    ...
    const rarity = UpgradesService.recipeExandedUpgradeData[material]?.rarity ?? undefined;
    if (hasElite) {
  ...
  public static getEnergyCost(type: CampaignType): number {
    switch (type) {
      case CampaignType.Normal: return 6;
      case CampaignType.Mirror: return 6;
      case CampaignType.Early: return 5;
      case CampaignType.SuperEarly: return 3;
      case CampaignType.Elite: return 10;
    }
    return -1;
  }
   ...
   *          energy spent.
   */
  public static getCostToFarmMaterial(
    type: CampaignType,
    count: number,
   ...
   */
  public static getFarmableLocations(material: string, campaignProgress: ICampaignsProgress): ICampaignBattleComposed[] {
    var result: ICampaignBattleComposed[] = [];

    Object.entries(CampaignsService.getCampaignComposed()).forEach(([campaign, battle]) => {
      if (battle.reward == material &&
   ...
   */
  public static getUnfarmableLocations(material: string, campaignProgress: ICampaignsProgress): ICampaignBattleComposed[] {
    var result: ICampaignBattleComposed[] = [];
