...
export const filter = (characters: ICharacter[]) => ({
    ...
    byDamageType: (damageType: DamageType, not = false) => characters.filter(char => not ? char.damageTypes.all.every(type => type !== damageType) : char.damageTypes.all.some(type => type === damageType)),
    byTrait: (trait: Trait, not = false) => characters.filter(char => not ? char.traits.every(type => type !== trait) : char.traits.some(type => type === trait)),
    ...
    isMechanical: () => characters.filter((char) => char.traits.some(type => type === Trait.LivingMetal || type === Trait.Mechanical)),
    isNotMechanical: () => characters.filter((char) => char.traits.every(type => type !== Trait.LivingMetal && type !== Trait.Mechanical)),
});
