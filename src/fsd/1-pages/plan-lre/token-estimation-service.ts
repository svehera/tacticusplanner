...
export class TokenEstimationService {
    ...
    static markRestrictionsAsCleared(token: TokenUse, track: ILreTrackProgress): void {
        ...
        for (const restriction of token.restrictionsCleared) {
            const requirement = battle.requirementsProgress.find(request => request.id === restriction.id);
    ...
    static computeHighestAvailableBattle(track: ILreTrackProgress): number {
        ...
        for (let index = 0; index < track.battles.length; ++index) {
            const request = track.battles[track.battles.length - 1 - index].requirementsProgress.find(
                request_ => request_.id === killPointsId
            );
            if (request === undefined) {
                console.error("couldn't find a kill-points requirement");
                return -1;
            }
            if (!request.completed) return index;
        }
    ...
    public static computeCurrentProgress(
        ...
        if (starIndex < ascensionMilestones.length) {
            addlShardsForNextMilestone = ascensionMilestones[starIndex].incrementalShards - currentShards;
    ...
    private static computeTotalPointsToNextMilestone(
        ...
        if (nextPointMilestoneIndex < pointMilestones.length) {
            return pointMilestones[nextPointMilestoneIndex - 1].points;
        }
        return Infinity;
    }
