...
export class LeProgressService {
    ...
    private static computePointsForNextMilestone(
        ...
        return model.pointsMilestones.at(-1)!.cumulativePoints;
    }
    ...
    private static convertLaneToTrack(
        ...
        for (const [index, objective] of externalTrack.battleConfigs[0].objectives.entries()) {
            ...
            const request = eventTrack.unitsRestrictions.find(
            ...
            const requestIndex = track.requirements.findIndex(x => x.id === request.name);
            if (requestIndex === -1) {
                throw new Error(
                    `Cannot find requirement index for ID ${request.name} in track ${eventTrack.name} of event ${eventId}.`
                );
            }
            restrictionIndexMap[index] = requestIndex;
        }
        ...
        for (const battle of returnValue.battles) {
            ...
            for (const requestProgress of battle.requirementsProgress) {
                requestProgress.completed = false;
                requestProgress.highScore = undefined;
                requestProgress.killScore = undefined;
                requestProgress.blocked = false;
                if (
                    requestProgress.status !== RequirementStatus.MaybeClear &&
                    requestProgress.status !== RequirementStatus.StopHere
                ) {
                    requestProgress.status = RequirementStatus.NotCleared;
                }
            }
        }
        ...
        for (const [battleIndex, progress] of externalTrack.progress.entries()) {
            ...
            if (progress.objectivesCleared.length === 6) {
                ...
                battle.totalPoints = sum(battle.requirementsProgress.map(request => request.points));
                for (const requestProgress of battle.requirementsProgress) {
                    requestProgress.completed = true;
                    requestProgress.blocked = false;
                    requestProgress.status = RequirementStatus.Cleared;
                }
                continue;
            }
            ...
            if (progress.objectivesCleared.includes(0)) {
                for (const requestId of ['_defeatAll', '_killPoints', '_highScore']) {
                    const requestProgress = battle.requirementsProgress.find(x => x.id === requestId)!;
                    requestProgress.completed = true;
                    requestProgress.blocked = false;
                    requestProgress.status = RequirementStatus.Cleared;
                    requestProgress.killScore = undefined;
                    requestProgress.highScore = undefined;
                    battle.totalPoints += requestProgress.points;
                }
            } else {
            ...
            for (const objectiveIndex of progress.objectivesCleared) {
                ...
                const requestIndex = restrictionIndexMap[objectiveIndex];
                const requestProgress = battle.requirementsProgress[requestIndex];
                if (requestProgress === undefined) {
