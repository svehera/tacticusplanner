...
describe('LreRequirementStatusService', () => {
    describe('getRequirementStatus', () => {
        ...
        it('should return status from new status field when available', () => {
            const request = createRequirementProgress('req1', RequirementStatus.MaybeClear);
            const battle = createBattleProgress(0, [request]);
            const tracksProgress = [createTrackProgress('alpha', [battle])];
        ...
        it('should return Cleared when legacy completed flag is true', () => {
            const request = createRequirementProgress('req1', undefined, true, false);
            const battle = createBattleProgress(0, [request]);
            const tracksProgress = [createTrackProgress('alpha', [battle])];
        ...
        it('should return StopHere when legacy blocked flag is true', () => {
            const request = createRequirementProgress('req1', undefined, false, true);
            const battle = createBattleProgress(0, [request]);
            const tracksProgress = [createTrackProgress('alpha', [battle])];
        ...
        it('should return NotCleared when both legacy flags are false', () => {
            const request = createRequirementProgress('req1', undefined, false, false);
            const battle = createBattleProgress(0, [request]);
            const tracksProgress = [createTrackProgress('alpha', [battle])];
        ...
        it('should prioritize new status field over legacy flags', () => {
            const request = createRequirementProgress('req1', RequirementStatus.PartiallyCleared, true, false);
            const battle = createBattleProgress(0, [request]);
            const tracksProgress = [createTrackProgress('alpha', [battle])];
        ...
        it('should return NotCleared status (0) even when legacy completed flag is true', () => {
            const request = createRequirementProgress('req1', RequirementStatus.NotCleared, true, false);
            const battle = createBattleProgress(0, [request]);
            const tracksProgress = [createTrackProgress('alpha', [battle])];
        ...
        it('should find the correct requirement across multiple tracks and battles', () => {
            const request1 = createRequirementProgress('req1', RequirementStatus.Cleared);
            const request2 = createRequirementProgress('req2', RequirementStatus.StopHere);
            const battle0 = createBattleProgress(0, [request1]);
            const battle1 = createBattleProgress(1, [request2]);
