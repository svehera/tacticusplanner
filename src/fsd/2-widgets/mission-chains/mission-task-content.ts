...
/**
 * Resolves one mission task's description content. Templates are hand-authored (no source strings
 * exist for these locaKeys anywhere in the extracted data) using the same `<style="X">{var}</style>`
 ...
 */
export function getMissionTaskContent(task: IMissionTask): TaskContent {
    const builder = TASK_BUILDERS[task.name];
    return builder ? builder(task) : plain(`${task.name.replaceAll(/([a-z0-9])([A-Z])/g, '$1 $2')} — {target}`);
}
