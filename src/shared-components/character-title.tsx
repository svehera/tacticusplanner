...
export const CharacterTitle =  ({ character, showLockedWithOpacity, onClick, short, imageSize }: { character: ICharacter, showLockedWithOpacity?: boolean, onClick?: () => void, short?: boolean, imageSize?: number }) => {
    ...
    const characterFull = (<div  style={{ display: 'flex', gap: 10, alignItems: 'center', opacity, cursor }} onClick={onClick}>
    ...
    const characterShort =  (<div  style={{ display: 'flex', gap: 10, alignItems: 'center', opacity, cursor }} onClick={onClick}>
        <Tooltip content={character.name} relationship={'description'} hideDelay={1000}>
            <span style={{ height: imageSize }}><CharacterImage key={character.name} character={character} imageSize={imageSize}/></span> 
        </Tooltip>
        <span>{character.name}</span>
    </div>);
